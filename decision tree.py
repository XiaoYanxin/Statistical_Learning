...
def calc_ent(x):
    ...
    for x_value in x_value_list:
        ...
        ent -= p* logp
    return ent
def calc_condition_ent(x,y):#H(y|x)
    x_value_list = set([x[i] for i in range(x.shape[0])])
    ent = 0.0
    for x_value in x_value_list:
        sub_y = y[x==x_value]
        temp_ent = calc_ent(sub_y)
        ent += (float(x[x==x_value].shape[0])/x.shape[0])*temp_ent
    return ent
...
if __name__ == '__main__':
    ...
    raw_data = pd.read_csv('/home/xiaoyx/PycharmProjects/statistical learning/mnist_data/mnist_train.csv', header=0)
